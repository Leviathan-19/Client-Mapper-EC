...
export const VisitsList: React.FC<any> = ({ navigation }) => {
  ...
  const getStatusColor = (status: string | undefined) => {
    switch (status) {
      case 'no_interesado': return '#c73737';
      case 'por_visitar': return '#eab308';
      case 'atendido': return '#28a745';
  ...
  const renderListItem = ({ item }: { item: VisitaMapItem }) => (
    <TouchableOpacity style={styles.card} onPress={() => handlePinPress(item)}>
      ...
      <View style={[styles.statusBadge, { backgroundColor: getStatusColor(item.estado_comercial) }]}>
        <Text style={styles.statusText}>{item.estado_comercial || 'desconocido'}</Text>
      </View>
      {item.ultima_visita && (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        {viewMode === 'list' ? (
          ...
          <MapView items={visitasMapItems} onPinPress={handlePinPress} />
        )}
      </View>
      ...
      <Modal visible={!!selectedItem} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            {selectedItem && (
              <>
                <Text style={styles.modalTitle}>{selectedItem.nombre_comercial}</Text>
                <Text style={styles.modalSubtitle}>{selectedItem.direccion}</Text>
                
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 15 }}>
                  <Text style={styles.modalText}>Estado comercial: </Text>
                  <View style={[styles.statusBadge, { backgroundColor: getStatusColor(selectedItem.estado_comercial), marginTop: 0 }]}>
                    <Text style={styles.statusText}>{selectedItem.estado_comercial}</Text>
                  </View>
                </View>
                ...
                {selectedItem.cliente_id && (
                  <>
                    ...
                    <Text style={styles.modalText}>{selectedItem.cliente_nombre}</Text>
                  </>
                )}
                ...
                {selectedItem.ultima_visita ? (
                  <>
                    ...
                    <Text style={styles.modalText}>Estado: {selectedItem.ultima_visita.estado_visita}</Text>
                    {selectedItem.ultima_visita.fecha_programada && (
                      <Text style={styles.modalText}>Programada para: {new Date(selectedItem.ultima_visita.fecha_programada).toLocaleDateString()}</Text>
                    )}
                  </>
                ) : (
                ...
                <View style={styles.modalButtons}>
                  <TouchableOpacity style={[styles.modalButton, styles.cancelButton]} onPress={closeDetail}>
                    <Text style={styles.modalButtonText}>Cerrar</Text>
                  </TouchableOpacity>
                  
                  {selectedItem.ultima_visita && selectedItem.ultima_visita.estado_visita === 'programada' && (
                    <TouchableOpacity style={[styles.modalButton, styles.checkInButton]} onPress={handleCheckIn}>
                      <Text style={styles.modalButtonText}>Check-in GPS</Text>
                    </TouchableOpacity>
                  )}
                  
                  {selectedItem.ultima_visita && selectedItem.ultima_visita.estado_visita === 'en_curso' && (
                    <TouchableOpacity style={[styles.modalButton, styles.completeButton]} onPress={handleComplete}>
